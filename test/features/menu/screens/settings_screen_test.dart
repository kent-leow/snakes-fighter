...
void main() {
  group('SettingsScreen', () {
    ...
    testWidgets('should show reset dialog when reset button is pressed', (
      tester,
    ) async {
      await tester.pumpWidget(
        MaterialApp(home: SettingsScreen(settingsService: settingsService)),
    ...
    testWidgets('should handle null settings service gracefully', (
      tester,
    ) async {
      await tester.pumpWidget(const MaterialApp(home: SettingsScreen()));

      // Initially should show loading indicator or settings content
      await tester.pump();

      // Either loading or fully loaded state is acceptable
      final hasLoading = find
          .byType(CircularProgressIndicator)
          .evaluate()
          .isNotEmpty;
      final hasSettings = find.text('Audio & Haptics').evaluate().isNotEmpty;

      expect(hasLoading || hasSettings, true);
    });

    testWidgets('should create settings service if not provided', (
      tester,
    ) async {
      await tester.pumpWidget(const MaterialApp(home: SettingsScreen()));
    ...
    group('Icon Tests', () {
      ...
      testWidgets('should show correct icons for vibration states', (
        tester,
      ) async {
        await tester.pumpWidget(
          MaterialApp(home: SettingsScreen(settingsService: settingsService)),
