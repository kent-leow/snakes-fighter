  ...
  group('Real-time Sync Performance Tests', () {
    ...
    group('Latency Requirements', () {
      test('event broadcasting latency is under 10ms', () async {
        ...
        final testEvents = List.generate(100, (index) => PlayerMoveEvent(
          playerId: 'player1',
        ...
        for (int i = 0; i < receivedEvents.length; i++) {
          ...
          expect(latency.inMilliseconds, lessThan(10), 
      ...
      test('delta calculation performance under 5ms for typical game state', () {
        ...
        final gameState = GameState(
          ...
          snakes: {
            ...
            'player2': const Snake(
              positions: [
                Position(15, 15), Position(14, 15), Position(13, 15)
              ],
        ...
        final avgTimePerUpdate = stopwatch.elapsedMicroseconds / 100 / 1000; // Convert to milliseconds
        expect(avgTimePerUpdate, lessThan(5.0), 
               reason: 'Average delta calculation time was ${avgTimePerUpdate}ms');
      });

      test('bandwidth optimization - delta size under 1KB for typical updates', () {
        ...
        final movementUpdate = initialState.copyWith(
          snakes: {
            ...
            'player1': initialState.snakes['player1']!.copyWith(
              positions: [const Position(5, 4), const Position(5, 5), const Position(5, 6)],
            ),
          },
        );
        ...
        final initialDelta = deltaService.calculateDelta(roomId, initialState);
        final movementDelta = deltaService.calculateDelta(roomId, movementUpdate);
    ...
    group('Scalability Tests', () {
      ...
      test('maintains performance with large snake positions', () {
        ...
        final longSnakePositions = List.generate(100, (i) => Position(i % 20, i ~/ 20));
        ...
        for (int i = 0; i < 10; i++) {
          final updatedPositions = [
            Position(longSnakePositions.first.x + 1, longSnakePositions.first.y),
    ...
    group('Memory Efficiency', () {
      ...
      test('broadcaster properly disposes room streams', () {
        ...
        for (int i = roomsToDispose; i < numRooms; i++) {
          expect(() => broadcaster.broadcastEvent('room$i', PlayerMoveEvent(
            playerId: 'test',
            timestamp: DateTime.now().millisecondsSinceEpoch,
            direction: Direction.up,
