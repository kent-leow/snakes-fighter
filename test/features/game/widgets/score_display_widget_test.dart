...
void main() {
  group('ScoreDisplayWidget', () {
    ...
    testWidgets('should hide high score when showHighScore is false', (
      tester,
    ) async {
      await tester.pumpWidget(
        MaterialApp(
    ...
    testWidgets('should hide high score when high score is zero', (
      tester,
    ) async {
      await tester.pumpWidget(
        MaterialApp(
    ...
    testWidgets('should use custom text styles', (tester) async {
      ...
      // Find the AnimatedDefaultTextStyle widgets and check their styles
      final scoreAnimatedStyle = tester.widget<AnimatedDefaultTextStyle>(
        find
            .ancestor(
              of: find.text('Score: 50'),
              matching: find.byType(AnimatedDefaultTextStyle),
            )
            .first,
      );

      final highScoreAnimatedStyle = tester.widget<AnimatedDefaultTextStyle>(
        find
            .ancestor(
              of: find.text('Best: 100'),
              matching: find.byType(AnimatedDefaultTextStyle),
            )
            .first,
      );

      expect(scoreAnimatedStyle.style.fontSize, 24);
      expect(scoreAnimatedStyle.style.color, Colors.red);
      expect(highScoreAnimatedStyle.style.fontSize, 16);
      expect(highScoreAnimatedStyle.style.color, Colors.blue);
    });
  });
