...
class SettingsService {
  ...
  double get audioVolume => _prefs.getDouble(_audioVolumeKey) ?? 1.0;

  /// Set the audio volume level (0.0 to 1.0).
  Future<void> setAudioVolume(double volume) async {
    await _prefs.setDouble(_audioVolumeKey, volume.clamp(0.0, 1.0));
  }
  ...
  GameDifficulty get difficulty {
    final difficultyIndex =
        _prefs.getInt(_difficultyKey) ?? 1; // Default to normal
    if (difficultyIndex >= 0 &&
        difficultyIndex < GameDifficulty.values.length) {
