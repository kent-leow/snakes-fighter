...
class PerformanceMonitor {
    ...
    return {
      ...
      'fps': {
        ...
        'average': _calculateAverage(
          _frameTimes.map((d) => 1000000.0 / d.inMicroseconds).toList(),
        ),
      },
      'memory': {
        'currentMB': memoryMB,
        'peakMB': _memoryUsage.isEmpty
            ? 0
      ...
      'network': {
        ...
        'maxLatencyMs': _networkLatency.isEmpty
            ? 0
            : _networkLatency.reduce((a, b) => a > b ? a : b),
      ...
      'customMetrics': _customMetrics.keys
          .map(
            (name) => {
              'name': name,
  ...
  List<String> getPerformanceWarnings() {
    ...
    if (memoryMB > _maxMemoryUsageMB) {
      warnings.add(
        'Memory usage above limit: ${memoryMB.toStringAsFixed(1)}MB > ${_maxMemoryUsageMB}MB',
      );
    }
    ...
    if (latencyMs > _maxNetworkLatencyMs) {
      warnings.add(
        'Network latency above limit: ${latencyMs.toStringAsFixed(1)}ms > ${_maxNetworkLatencyMs}ms',
      );
    }
