...
class MultiplayerGameEngine {
    ...
    if (winResult.isGameEnded) {
      ...
      return GameUpdateResult(
        ...
        metadata: {
          ...
          'player_rankings': winResult.playerRankings?.map((r) => r.toString()).toList(),
        },
      );
    }
  ...
  SnakeMoveResult _calculateSnakeMove(MultiplayerSnake snake) {
    ...
    final newHeadPosition = Position(
      snake.head.x + dx,
      snake.head.y + dy,
    );
  ...
  Map<String, dynamic> getGameStateSnapshot() {
    return {
      'snakes': _snakes.map((id, snake) => MapEntry(id, {
        'positions': snake.positions.map((p) => {'x': p.x, 'y': p.y}).toList(),
        'direction': snake.direction.name,
        'alive': snake.alive,
        'score': snake.score,
      })),
      'food': _currentFood != null ? {'x': _currentFood!.x, 'y': _currentFood!.y} : null,
