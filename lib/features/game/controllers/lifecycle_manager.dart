...
class LifecycleManager extends ChangeNotifier {
  ...
  Future<void> startGame() async {
    ...
    try {
      ...
      if (!success) {
        throw StateError('Cannot start game from current state: ${_stateManager.currentState}');
      }
  ...
  Future<void> pauseGame() async {
    ...
    try {
      ...
      if (!success) {
        throw StateError('Cannot pause game from current state: ${_stateManager.currentState}');
      }
  ...
  Future<void> resumeGame() async {
    ...
    try {
      ...
      final success = _stateManager.transitionTo(GameState.playing);
      if (!success) {
        throw StateError('Cannot resume game from current state: ${_stateManager.currentState}');
      }
  ...
  Future<void> endGame({int? finalScore}) async {
    ...
    try {
      ...
      // Update session stats
      if (finalScore != null) {
        _sessionStats['totalScore'] = (_sessionStats['totalScore'] ?? 0) + finalScore;
        _sessionStats['highScore'] = 
            (finalScore > (_sessionStats['highScore'] ?? 0)) ? finalScore : _sessionStats['highScore'];
      }
      
      // Transition to game over state
      final success = _stateManager.transitionTo(GameState.gameOver);
      if (!success) {
        throw StateError('Cannot end game from current state: ${_stateManager.currentState}');
      }
  ...
  Future<void> restartGame() async {
    ...
    try {
      ...
      if (!success) {
        throw StateError('Cannot restart game from current state: ${_stateManager.currentState}');
      }
  ...
  Future<void> returnToMenu() async {
    ...
    try {
      ...
      if (!success) {
        throw StateError('Cannot return to menu from current state: ${_stateManager.currentState}');
      }
